...
This study is based on SOCATv2019.tsv, thus excluding flag E data stored in a seperate file.
...
maps_SOCAT_all %>% 
  ...
  mapWorld+
  geom_tile()+
  scale_fill_viridis_c(name = "SST (°C)", option = "inferno")+
...
maps_SOCAT_all %>% 
  ...
  mapWorld+
  geom_tile()+
  scale_fill_viridis_c(name = expression(fCO[2]~(µatm)), limits = c(200, 500))+
  labs(x="Lon (°E)", y="Lat (°N)")+
  scale_x_continuous(breaks = seq(-180, 180, 30), limits = c(-170, 190.5))+
  scale_y_continuous(breaks = seq(-90, 90, 30), limits = c(-80, 90))+
  coord_quickmap(expand = FALSE)+
...
maps_SOCAT_all %>% 
  ...
  mapWorld+
  geom_tile()+
  scale_fill_divergent(name = expression(Delta~T[SST-equi]~(K)))+
  labs(x="Lon (°E)", y="Lat (°N)")+
  scale_x_continuous(breaks = seq(-180, 180, 30), limits = c(-170, 190.5))+
  scale_y_continuous(breaks = seq(-90, 90, 30), limits = c(-80, 90))+
  coord_quickmap(expand = FALSE)+
...
SST_lat_pattern %>% 
  ...
  scale_fill_viridis_c(name = "lg (nr obs)")+
  scale_y_continuous(breaks = seq(-90, 90, 30), limits = c(-80, 90))+
  labs(y="Lat (°N)", x="SST (K)")
...
fCO2rec is than calculated with a temperature and pCO~2~ dependent conversion factor.
...
df <- df %>%
  mutate(fCO2equ = fCO2rec / exp(0.0423*d_Tem),
         ...
         d_fCO2 = fCO2rec - fCO2rec_Mueller)
...
maps_SOCAT_all %>% 
  ggplot(aes(lon_int, lat_int, fill=t_coeff))+
  mapWorld+
  geom_tile()+
  scale_fill_divergent(name = expression(dln(pCO[2])/dT), midpoint = 0.0423)+
  labs(x="Lon (°E)", y="Lat (°N)")+
  scale_x_continuous(breaks = seq(-180, 180, 30), limits = c(-170, 190.5))+
  scale_y_continuous(breaks = seq(-90, 90, 30), limits = c(-80, 90))+
  coord_quickmap(expand = FALSE)+
...
maps_SOCAT_all %>% 
  ...
  mapWorld+
  geom_tile()+
  scale_fill_divergent(name = expression(Delta~pCO["2,SOCAT-Mueller"]~(µatm)))+
  labs(x="Lon (°E)", y="Lat (°N)")+
  scale_x_continuous(breaks = seq(-180, 180, 30), limits = c(-170, 190.5))+
  scale_y_continuous(breaks = seq(-90, 90, 30), limits = c(-80, 90))+
  coord_quickmap(expand = FALSE)+
